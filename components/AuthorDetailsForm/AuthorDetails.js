...
const AuthorDetails = () => {
    ...
    async function prevDetails(id) {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_APIBASE}/author/id/${id}`,
          {
            method: "GET",
  ...
  return (
    <>
      {showModal && (
        <MODAL heading={heading} message={message} changeState={setShowModal} />
      )}
      <div className={styles.formContainer}>
        <form
          onSubmit={handleSubmit}
          className="my-24 w-full max-w-4xl mx-auto"
        >
          <div className="flex flex-wrap -mx-3 mb-6">
            <div className="w-full px-3">
              <label
                className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2"
                htmlFor="grid-post-date"
              >
                NAME
              </label>
              <input
          ...
          <div className="flex flex-wrap -mx-3 mb-6">
            <div className="w-full px-3">
              ...
              <textarea
                ...
                className="appearance-none block w-full border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 min-h-max resize-none"
                id="grid-caption"
          ...
          <div
            className={
              "flex flex-wrap justify-center" + " " + styles.imgContainer
            }
          >
            <Image
              src={image}
