...
export default function Blog({ blogData }: { blogData: apiResponse }) {
	if (blogData.message) {
		return (
			<div className={styles.errorContainer}>
				...
				error message -
				<code className={styles.errorCode}>{blogData.message}</code>
			</div>
		);
	}
	...
	return (
		<>
			...
			<div
				className={`${styles.blogContainer} p-8 sm:px-16 md:px-24 lg:px-32 xl:px-40 2xl:px-48 my-5 mx-auto`}>
				...
				<div
					className={`${styles.thumbnail} relative rounded-lg border border-slate-200 shadow-sm overflow-hidden`}>
					<Image
						src={
							isValidURL(blogData.thumbnail)
								? blogData.thumbnail
				...
				<div className="flex flex-row gap-2 flex-wrap">
					{blogData.tags.map((tag) => (
						<div
							key={tag}
							className="block uppercase tracking-wide text-gray-700 text-xs font-bold shadow-sm mb-2 bg-gray-200 p-4 rounded-md">
